...
def hide_ground_props(states, threshold=0.84, keep=("EARTH_LIMB",)):
    """Key hide_render/hide_viewport on everything in PAD, SITE and the light poles
    from the first frame with altitude > threshold. 0.84 is the first frame (222)
    after the last prop, the tallest flood-light mast, has dropped out of the bottom
    of the shot. The original 0.25 (frame 203) cut the tower while it was still
    mid-frame, a visible pop in the climb; 0.64 (frame 215) fixed the tower but cut
    the masts mid-frame instead. Between the limb fade-in (~211) and 221 the masts
    slide down past the horizon glow as foreground poles, which reads correctly."""
